...
function IssuePage() {
    ...
    const trimTextToMaxWidth = (text, maxWidth) => {
        ...
        return (
            <span>
                {trimmedText}
                <span style={{ color: 'blue', fontSize: '15px' }}>{words.length > trimmedWords.length ? ' ...' : ''}</span>
            </span>
        );
    };
    ...
    return (
        <Box className={styles.bodybox}>
            <List>
                ...
                <ListItem className={styles.list}>
                    <Flex>
                        <Text fontSize='2xl'>Issues Management -  <Link href='/adminpages/issueManager' style={{ color: '#4d9ffe', textDecoration: 'none' }}>
                            Overview
                        </Link></Text>
                        <Spacer />
                        <Input
